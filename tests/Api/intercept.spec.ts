...
//web app--> intercept the network calls and logg them..
...
test('@smoke intercept and log requests',async({page})=>{
    ...
    await page.route('**/*',async (route)=>{
        ...
        route.request().url());
        await route.continue();//url1--capture and continuee./..url2--capture--continue
    });
...
test.skip('@regression mock search page with fake HTML', async({page})=>{
